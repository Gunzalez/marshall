<div class="footer">

    <ul class="site">
        <li><a href="index.php" title="Home">Home</a></li>
        <li><a href="#" title="Products">Products</a></li>
        <li><a href="configure-machine.php" title="Configure a Marshall Machine">Price &amp; Configure</a></li>
        <li><a href="spares.php" title="Spares">Spares</a></li>
        <li><a href="#" title="Stock Clearance">Stock Clearance</a></li>
        <li><a href="downloads.php" title="Downloads">Downloads</a></li>
        <li><a href="news.php" title="News">News</a></li>
        <li><a href="#" title="About Us">About us</a></li>
        <li><a href="#" title="Contact">Contact</a></li>
        <li><a href="#" title="Search the our site">Search</a></li>
    </ul>

    <ul class="aux">
        <li><a href="#" title="Site Map">Site Map</a></li>
        <li><a href="#" title="Privacy and Cookie policies">Privacy &amp; Cookies</a></li>
        <li><a href="#" title="T&amp;C">T&amp;C</a></li>
    </ul>

    <div class="address">
        <p>Charles J Marshall (Aberdeen) Limited, <span>Chapel Works, Bucksburn, Aberdeen, AB21&nbsp;9TL</span></p>
        <p>Tel:&nbsp;<a href="tel:+441224722777">(01224) 722777</a>, <span>Fax:&nbsp;(01224) 724034,</span> Email:&nbsp;<a href="mailto:charles@marshall-trailers.co.uk">charles@marshall-trailers.co.uk</a></p>
        <p>Registration Number: SC064659</p>
    </div>

    <div class="made-by">
        <p>Made by <a href="http://www.sugarshaker.com/" target="_blank" class="sugarshaker">SugarShaker</a></p>
    </div>

</div>